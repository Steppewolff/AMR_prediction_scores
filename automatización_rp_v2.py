...
def evaluate_condition(record, condition):
    ...
    if mut_type == "LOF":
        if evaluate_LOF(record):
            return eff * condition["simple_value"]
        else:
            return 0
        ...
        if result is None:
            return 0
        if result == "double":
            return eff * condition["doble_value"]
        else:
...
def evaluate_regulators(gene, condition, records):
    ...
    # Evaluar los sub-loci: verificar si existe al menos un registro que cumpla la condición del sublocus
    regulator_pass = False
    for sub in subloci:
        sub_cond = subloci[sub]
        sub_recs = [r for r in records if r[7] == sub]
        for rec in sub_recs:
            if evaluate_condition(rec, sub_cond) != 0:
                regulator_pass = True
                break
        if regulator_pass:
            break
    ...
    for rec in main_records:
        main_score += evaluate_condition(rec, condition)
        ...
        for ab, cond in conditions.items():
            ...
            if cond.get("regulators", "NO") == "NO":
                ...
                for rec in gene_records:
                    final_scores[ab] += evaluate_condition(rec, cond)
